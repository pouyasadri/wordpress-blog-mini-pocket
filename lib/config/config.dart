import 'package:flutter/material.dart';

class Config {
  static const appName = 'Live Punjabi TV';
  static const supportEmail = 'info@theminipocket.com';
  static const privacyPolicyUrl = 'https://theminipocket.com/terms-of-use/';
  static const iOSAppID = '000000';

  //SOCIAL LINKS
  static const String facebookPageUrl =
      'https://www.facebook.com/theminipocket';
  static const String youtubeChannelUrl = 'https://www.youtube.com/channel/';
  static const String twitterUrl = 'https://twitter.com/minipocket5';
  static const String instagramUrl = 'https://instagram.com/minipocketindia/';

  //app theme color
  //final Color appThemeColor = Color(0xFFf8bd7f);
  final Color appThemeColor = Color(0xFFed3966);
  //#ed3966

  // Icons
  static const String appIcon = 'assets/images/icon.png';
  static const String logo = 'assets/images/logo.png';
  static const String splash = 'assets/images/splash.png';

  //languages
  static const List<String> languages = [
    'English',
    'Spanish',
    'Arabic',
    'Hindi',
    'Punjabi'
  ];

  //Image Assets
  static const String commentImage = "assets/images/comment.svg";
  static const String bookmarkImage = "assets/images/bookmark.svg";
  static const String notificationImage = "assets/images/notification.svg";
  static const String noContentImage = "assets/images/no_content.svg";

  //animation files
  static const String doneAnimation = 'assets/animation_files/done.flr';
  static const String searchAnimation = 'assets/animation_files/search.flr';
}
