...
class WordPressService {
  ...
  Future fetchPostsByCategoryIdExceptPostId(int? postId, int? catId, int contentAmount) async {
    ...
    var response = await http.get(Uri.parse(
        "${WpConfig.websiteUrl}/wp-json/wp/v2/posts?exclude=$postId&categories[]=$catId&per_page=$contentAmount"),headers: headers);
    List? decodedData = jsonDecode(response.body);
  ...
  Future fetchPostsBySearch(String searchText) async {
    ...
    var response = WpConfig.blockedCategoryIds.isEmpty
     ? await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?per_page=30&search=$searchText"),headers: headers)
     : await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/posts?per_page=30&search=$searchText&categories_exclude=" + WpConfig.blockedCategoryIds),headers: headers);
    List? decodedData = jsonDecode(response.body);
    List<Article>? articles;
  ...
  Future fetchCommentsById(int? id) async {
    ...
    var response = await http.get(Uri.parse("${WpConfig.websiteUrl}/wp-json/wp/v2/comments?per_page=100&post=" + id.toString()),headers: headers);
    List? decodedData = jsonDecode(response.body);
